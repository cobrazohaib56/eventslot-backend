...
async function bookSlotAtomic(req: AuthRequest, res: Response): Promise<void> {
  ...
  const slot = await TimeSlot.findOneAndUpdate(
    ...
    { $set: { isBooked: true, bookedBy: req.user!.id, bookedByName: req.user!.name } },
    { new: false } // return the original doc to check it existed
  );
...
async function bookSlotWithTransaction(req: AuthRequest, res: Response): Promise<void> {
  ...
  try {
    ...
    slot.isBooked = true;
    slot.bookingId = booking._id as mongoose.Types.ObjectId;
...
router.post('/', requireRole('user'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.get('/event/:eventId', requireRole('host'), async (req: AuthRequest, res: Response): Promise<void> => {
  try {
