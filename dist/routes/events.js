...
router.get('/host/my-events', requireRole('host'), async (req, res) => {
    try {
...
router.get('/:id/slots/available', async (req, res) => {
    try {
...
router.put('/:id', requireRole('host'), async (req, res) => {
    try {
        ...
        const { title, description, duration, startDate, endDate, dailyStartTime, dailyEndTime } = req.body;
        const scheduleChanged = (startDate && startDate !== event.startDate.toISOString()) ||
        ...
        if (dailyStartTime)
            event.dailyStartTime = dailyStartTime;
        if (dailyEndTime)
            event.dailyEndTime = dailyEndTime;
        await event.save();
        if (scheduleChanged) {
            await generateTimeSlots(event);
        }
...
router.delete('/:id', requireRole('host'), async (req, res) => {
    try {
        ...
        const now = new Date();
        await Booking.updateMany({ eventId: event._id, status: 'confirmed', startTime: { $gt: now } }, { status: 'cancelled', cancelledAt: now });
        // Free the slots for cancelled bookings
        await TimeSlot.updateMany({ eventId: event._id, isBooked: true }, { isBooked: false, bookingId: null, bookedBy: null, bookedByName: null });
